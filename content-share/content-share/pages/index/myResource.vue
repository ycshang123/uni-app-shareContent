<template>
	<view>
		<view class="item-row" v-for="(item, index) in items" :key="index">
			...
			<view class="score"><view class="content-title" @click="goDetail(item.downloadUrl, item.id)">下载</view></view>
		</view>
	</view>
</template>
...
export default {
	...
	methods: {
		getShare: function() {
			uni.request({
				url: BASE_API_URL + '/shares/exchangeList/' + uni.getStorageSync('user').id,
				method: 'GET',
				header: {
